...
module Tamashi
  module Manager
    class Client
      ...
      def send(packet)
        packet = packet.dump if packet.is_a?(Tamashi::Packet)
      ...
      def accept(type, id)
        ...
        send(Tamashi::Packet.new(Tamashi::Type::AUTH_RESPONSE, @channel.id, true).dump)
      end
      ...
      def receive(data)
        ...
        Tamashi::Resolver.resolve(Tamashi::Packet.load(data), client: self)
      rescue AuthorizationError => e
        Manager.logger.error("Client #{id} authentication failed => #{e.message}")
        send(Tamashi::Packet.new(Tamashi::Type::AUTH_RESPONSE, 0, false))
        @driver.close
      rescue => e
