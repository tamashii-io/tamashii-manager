...
RSpec.describe Tamashi::Manager::ChannelPool do
  let(:pool_size) { 10 }
  ...
  describe "#ready" do
    it "can add to ready pool when channel has client" do
      ...
      channel.add(double(Tamashi::Manager::Client))
  ...
  describe "#get_idle" do
    it { expect(subject.get_idle).to be_instance_of(Tamashi::Manager::Channel) }
